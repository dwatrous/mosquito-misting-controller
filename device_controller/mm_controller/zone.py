    ...
    def execute_spray(self, skip_override=False, spray_event="SCHEDULE", water_only=False):
        if not self.spraying:
            ...
            if (low_temp_last_24hr < self.low_temp_threshold_f or low_temp_next_24hr < self.low_temp_threshold_f) and not skip_override:
                ...
                self.zonecloud.send_message("Spray Skipped", "The scheduled spray in '%s' was skipped due to weather conditions (low temperature)." % self.name)
            ...
            if rain_prediction_next_24hr > self.rain_threshold_in and not skip_override:
                ...
                self.zonecloud.send_message("Spray Skipped", "The scheduled spray in '%s' was skipped due to weather conditions (heavy rain)" % self.name)
                self.zonecloud.write_spray_occurence_ds(self.spraydata)
                return
            ...
            self.zonecloud.send_message("Spray Executed", "A spray just ran in '%s' to eliminate mosquitos. Enjoy!" % self.name)
